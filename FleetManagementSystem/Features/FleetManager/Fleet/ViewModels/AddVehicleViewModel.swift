...
class AddVehicleViewModel: ObservableObject {
    ...
    func saveVehicle() async {
        ...
        let payload: [String: Any] = [
            ...
            "vehicle_name": vehicleName.trimmingCharacters(in: .whitespaces),
            "number_plate": licensePlate,
            "vin": vin.uppercased().trimmingCharacters(in: .whitespaces),
            "registration_no": rcNumber.uppercased().trimmingCharacters(in: .whitespaces),
            "brand": brand,
            ...
            "fuel_type": fuelType,
            "registration_date": sqlDateFormatter.string(from: registrationDate),
            "rc_expiry_date": sqlDateFormatter.string(from: rcExpiry),
            "puc_expiry_date": sqlDateFormatter.string(from: pucExpiry),
            "documents": [
